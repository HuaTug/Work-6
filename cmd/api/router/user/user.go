...
import (
	user "HuaTug.com/cmd/api/handlers/user"
...
func Register(r *server.Hertz) {
	...
	root := r.Group("/", rootMw()...)
	{
		_v1 := root.Group("/v1", _v1Mw()...)
		{
			...
			_user.POST("/login", append(_loginuserMw(), user.Login)...)
			_user.POST("/update", append(_updateuserMw(), user.UpdateUser)...)
			{
				_create := _user.Group("/create", _createMw()...)
				_create.POST("/", append(_createuserMw(), user.Register)...)
			}
			{
				_query := _user.Group("/query", _queryMw()...)
				_query.POST("/", append(_queryuserMw(), user.QueryUserInfo)...)
			}
		}
	}
}
